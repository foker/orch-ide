...
impl<'a> TerminalView<'a> {
    ...
    fn handle_wheel_scrolled(
        ...
        // Invert scroll direction for natural scrolling (macOS)
        match delta {
            ScrollDelta::Lines { y, .. } => {
        ...
        match event {
            ...
            iced::keyboard::Event::KeyPressed {
                ...
                Key::Character(k) => {
                    ...
                    // Fallback: if no binding matched and modifiers are active,
                    // try physical key (fixes Cmd+V with non-latin layouts)
                    if binding_action == BindingAction::Ignore && !state.keyboard_modifiers.is_empty() {
                        if let Some(latin_char) = key.to_latin(*physical_key) {
                            let latin_lower = latin_char.to_ascii_lowercase().to_string().into();
                            binding_action = self.term.bindings.get_action(
                                InputKind::Char(latin_lower),
                                state.keyboard_modifiers,
                                last_content.terminal_mode,
                            );
                        }
